...
def httpGet(url):
    ...
    p = proxy[random.randint(0,len(proxy) - 1)]
    #page = session.get(url, headers=headers)
    page = session.get(lianjia_community, proxies=p, headers=headers)
...
def communityHandler(community_url):
    ...
    content = requests.get(community_url)
    ...
    strategy_page = requests.get(strategy_url).content
    file_hand = open(location, 'w')
    file_hand.writelines(strategy_page)
    file_hand.close()
    print 'done'
...
#district_name = setDistrictName('http://sh.lianjia.com/xiaoqu/minhang/')
#print district_name
#pageHandler('http://sh.lianjia.com/xiaoqu/beicai/d1/')
#pageHandler('http://sh.lianjia.com/xiaoqu/shanghaizhoubian/d16')
#communityHandler('http://sh.lianjia.com/xiaoqu/5011102207057.html')
#os._exit(0)
